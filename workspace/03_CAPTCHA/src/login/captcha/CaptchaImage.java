...
public class CaptchaImage {
	...
	// 3. 매개변수 
	//			1) HttpServletRequest request(realPath를 구하기 위해서 필요하다)
	//			2) String key
    public static void getCaptchaImage(HttpServletRequest request, String key) { // login.java에 있는 key값을 가져온다(매개변수에 써주기)
    	
        String clientId = "xGuSNkdZ9cSzRx1wZNqe"; //애플리케이션 클라이언트 아이디값";
        String clientSecret = "0sRyCHBuMI"; //애플리케이션 클라이언트 시크릿값";
    ...
    private static String getImage(HttpServletRequest request, InputStream is){
        ...
        // realPath를 알아내려면 request가 필요하다(HttpServletRequest 사용)
        String realPath=request.getServletContext().getRealPath(DIR);
        
        // realPath에 디렉터리가 생성되지 않았다면 생성하는 코드 만들기
        ...
        try(OutputStream outputStream = new FileOutputStream(f)){
            ...
            // 여기에서 request에 데이터를 저장하면 Login.java에서도 저장된 데이터를 사용할 수 있다
            // 캡차 이미지 다운로드 성공한 지점
